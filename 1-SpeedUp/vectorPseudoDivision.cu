...
/*
 * Implementación de un programa en CUDA que realiza la división de dos vectores, entrada a entrada,
...
/*
 * Función que realiza la división de dos arreglos de números aleatorios utilizando la CPU.
 */
void divOnCPU(float *A, float *B, float *C, int size)
{
    for (int i = 0; i < size; i++)
    {
        C[i] = A[i] / B[i];
...
/*
 * 4. Kernel en CUDA que realiza la división de dos arreglos de números aleatorios utilizando la GPU.
 */
__global__ void divOnGPU(float *A, float *B, float *C, int size)
{
...
/*
 * Función que valida los resultados de la división, obtenidos en la GPU y CPU.
 */
void validate(float *res_GPU, float *res_CPU, int size)
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        {
            printf("ERROR: división distinta.\n");
            return;
        }
    }
...
int main()
{
    ...
    dim3 thread(1024);
    double tic = cpuTime();
    divOnGPU<<<block, thread>>>(d_A, d_B, d_C, N);
